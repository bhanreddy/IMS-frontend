...
export default function AcademicManagement() {
    ...
    const handleAdd = async () => {
        ...
        try {
            if (activeTab === 'classes') {
                ...
                await ResultService.createSubject({ name: newItemName, code: newItemCode });
            } else if (activeTab === 'mappings') {
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                {(['classes', 'sections', 'years', 'subjects', 'mappings'] as TabType[]).map((tab) => (
                    <TouchableOpacity
                        ...
                        style={[styles.tab, activeTab === tab && styles.activeTab, { minWidth: 100 }]}
                    >
    ...
    const handleDelete = async (id: string, name: string) => {
        Alert.alert(
            'Confirm Delete',
            `Are you sure you want to delete ${activeTab.slice(0, -1)} "${name}"? This action cannot be undone and will fail if there are linked dependencies.`,
            [
    ...
    const renderItem = ({ item, index }: { item: any; index: number }) => (
        ...
        >
            <View style={styles.itemInfo}>
                ...
                {activeTab === 'years' && (
                    <Text style={styles.itemSub}>{item.start_date} to {item.end_date}</Text>
                )}
            </View>
            <TouchableOpacity
                style={styles.deleteButton}
                onPress={() => handleDelete(item.id, item.name || item.code)}
            >
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.content}>
                {loading ? (
                    <ActivityIndicator size="large" color={ADMIN_THEME.colors.primary} style={{ marginTop: 50 }} />
                ) : (
                    <FlatList
                        data={
                            activeTab === 'classes' ? classes :
                                activeTab === 'sections' ? sections :
                                    activeTab === 'years' ? years :
                                        activeTab === 'subjects' ? subjects : mappings
                        }
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        {activeTab !== 'mappings' && (
                            <TextInput
                                style={styles.input}
                                placeholder={activeTab === 'years' ? "Code (e.g. 2023-24)" : "Code (optional)"}
                                value={newItemCode}
                        ...
                        {activeTab === 'mappings' && (
                            <>
                                <Text style={{ fontWeight: 'bold', marginBottom: 5 }}>Class</Text>
                                <ScrollView horizontal style={{ marginBottom: 10 }}>
                                    {classes.map(c => (
                                        <TouchableOpacity key={c.id} onPress={() => setSelClassId(c.id)} style={[styles.tab, { backgroundColor: selClassId === c.id ? '#6366F1' : '#ddd', paddingHorizontal: 10, marginRight: 5 }]}>
                                            <Text style={{ color: selClassId === c.id ? '#fff' : '#000' }}>{c.name}</Text>
                                        </TouchableOpacity>
                                    ))}
                                </ScrollView>
                                ...
                                <ScrollView horizontal style={{ marginBottom: 10 }}>
                                    {sections.map(s => (
                                        <TouchableOpacity key={s.id} onPress={() => setSelSectionId(s.id)} style={[styles.tab, { backgroundColor: selSectionId === s.id ? '#6366F1' : '#ddd', paddingHorizontal: 10, marginRight: 5 }]}>
                                            <Text style={{ color: selSectionId === s.id ? '#fff' : '#000' }}>{s.name}</Text>
                                        </TouchableOpacity>
                                    ))}
                                </ScrollView>

                                <Text style={{ fontWeight: 'bold', marginBottom: 5 }}>Academic Year</Text>
                                <ScrollView horizontal style={{ marginBottom: 10 }}>
                                    {years.map(y => (
                                        <TouchableOpacity key={y.id} onPress={() => setSelYearId(y.id)} style={[styles.tab, { backgroundColor: selYearId === y.id ? '#6366F1' : '#ddd', paddingHorizontal: 10, marginRight: 5 }]}>
                                            <Text style={{ color: selYearId === y.id ? '#fff' : '#000' }}>{y.code}</Text>
                                        </TouchableOpacity>
                                    ))}
                                </ScrollView>
                            </>
