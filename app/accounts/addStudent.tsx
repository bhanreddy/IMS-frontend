...
// Reusable Components
const InputField = ({ label, placeholder, value, onChangeText, keyboardType = 'default', icon, required = false, secureTextEntry = false }: any) => (
    <View style={styles.inputGroup}>
        <Text style={styles.label}>
            {label} {required && <Text style={{ color: ADMIN_THEME.colors.danger }}>*</Text>}
        </Text>
...
const SelectField = ({ label, value, options, onSelect, placeholder, icon, required = false, loading = false }: any) => {
    ...
    return (
        <View style={styles.inputGroup}>
            ...
            >
                <Ionicons name={icon} size={20} color={ADMIN_THEME.colors.text.muted} style={styles.inputIcon} />
                <Text style={[styles.input, !selectedOption && { color: ADMIN_THEME.colors.text.muted }, { paddingTop: 12 }]}>
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <FlatList
                            ...
                            renderItem={({ item }) => (
                                <TouchableOpacity
                                    style={[styles.optionItem, value?.toString() === item.id.toString() && styles.selectedOption]}
                                    onPress={() => {
                                        onSelect(item.id);
                                ...
                                >
                                    ...
                                    {value?.toString() === item.id.toString() && (
                                        <Ionicons name="checkmark" size={20} color={ADMIN_THEME.colors.primary} />
                                    )}
                                </TouchableOpacity>
                            )}
                        />
...
export default function AddStudentScreen() {
    ...
    const [formData, setFormData] = useState<CreateStudentRequest>({
        ...
        last_name: '',
        dob: '',
        gender_id: 1, // Default: Male
        admission_no: '',
        admission_date: new Date().toISOString().split('T')[0],
        status_id: 1, // Default: Active
        category_id: 1, // Default: General
        religion_id: 1, // Default: Hindu
        blood_group_id: 1, // Default: A+
        email: '',
        phone: '',
        password: '',
        role_code: 'student',
    ...
    const [guardian, setGuardian] = useState({ first_name: '', last_name: '', phone: '', relation: '', occupation: '' }); // Guardian needs custom relation? Or just 'Guardian'
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (father.first_name && father.last_name) {
                parents.push({ ...father, relation: 'Father' as const, is_primary: true });
            }
            if (mother.first_name && mother.last_name) {
                parents.push({ ...mother, relation: 'Mother' as const });
            }
            if (guardian.first_name && guardian.last_name) {
                parents.push({ ...guardian, relation: 'Guardian' as const, is_guardian: true });
            }

            const payload: CreateStudentRequest = { ...formData, parents };

            if (isEditMode) {
                await StudentService.update(id as string, payload as any);
                Alert.alert('Success', 'Student updated successfully!', [{ text: 'OK', onPress: () => router.back() }]);
            } else {
                await StudentService.create(payload);
                Alert.alert('Success', 'Student created successfully!', [{ text: 'OK', onPress: () => router.back() }]);
            }
        } catch (error: any) {
    ...
    const onAdmissionDateChange = (event: any, selectedDate?: Date) => {
        ...
        if (selectedDate) {
            setFormData({ ...formData, admission_date: selectedDate.toISOString().split('T')[0] });
        }
    };
        ...
        <View style={styles.container}>
            ...
            >
                <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                    ...
                    {/* Header Info Card */}
                    <LinearGradient
                        colors={[ADMIN_THEME.colors.primary, ADMIN_THEME.colors.secondary]}
                    ...
                    <Animated.View entering={FadeInDown.delay(100).duration(500)} style={styles.section}>
                        ...
                        <InputField
                            ...
                            onChangeText={(t: string) => setFormData({ ...formData, middle_name: t })}
                            icon="person-outline"
                        />
                        ...
                        <SelectField
                            ...
                            options={GENDERS}
                            onSelect={(id: number) => setFormData({ ...formData, gender_id: id })}
                        ...
                        <View style={styles.inputGroup}>
                            ...
                            >
                                ...
                                <Text style={[styles.input, !formData.dob && { color: ADMIN_THEME.colors.text.muted }, { paddingTop: 12 }]}>
                    ...
                    <Animated.View entering={FadeInDown.delay(200).duration(500)} style={styles.section}>
                        ...
                        <InputField
                            ...
                            onChangeText={(t: string) => setFormData({ ...formData, admission_no: t })}
                            icon="card-outline"
                            required={true}
                        />
                        ...
                        <InputField
                            ...
                            value={(formData as any).roll_number ? String((formData as any).roll_number) : 'Auto-generated'}
                            editable={false}
                            icon="list-outline"
                        />
                        ...
                        <View style={styles.inputGroup}>
                            ...
                            >
                                <Ionicons name="calendar-outline" size={20} color={ADMIN_THEME.colors.text.muted} style={styles.inputIcon} />
                                <Text style={[styles.input, !formData.admission_date && { color: ADMIN_THEME.colors.text.muted }, { paddingTop: 12 }]}>
                        ...
                        <SelectField
                            ...
                            options={classes}
                            onSelect={(id: string) => setFormData({ ...formData, class_id: id })}
                        ...
                        <SelectField
                            ...
                            options={sections}
                            onSelect={(id: string) => setFormData({ ...formData, section_id: id })}
                        ...
                        <SelectField
                            ...
                            onSelect={(id: number) => setFormData({ ...formData, status_id: id })}
                        ...
                        <SelectField
                            ...
                            onSelect={(id: string) => setFormData({ ...formData, academic_year_id: id })}
                    ...
                    <Animated.View entering={FadeInDown.delay(150).duration(500)} style={styles.section}>
                        ...
                        {/* Father */}
                        <Text style={[styles.label, { marginTop: 10, color: ADMIN_THEME.colors.primary }]}>Father's Details</Text>
                        <View style={styles.row}>
                            <View style={styles.halfInput}>
                                <InputField label="First Name" placeholder="Father Name" value={father.first_name} onChangeText={(t: string) => setFather({ ...father, first_name: t })} icon="person-outline" />
                            </View>
                            <View style={styles.halfInput}>
                                <InputField label="Last Name" placeholder="Surname" value={father.last_name} onChangeText={(t: string) => setFather({ ...father, last_name: t })} icon="person-outline" />
                            </View>
                        </View>
                        <InputField label="Phone" placeholder="Mobile Number" value={father.phone} onChangeText={(t: string) => setFather({ ...father, phone: t })} keyboardType="phone-pad" icon="call-outline" />
                        <InputField label="Occupation" placeholder="Designation" value={father.occupation} onChangeText={(t: string) => setFather({ ...father, occupation: t })} icon="briefcase-outline" />

                        {/* Mother */}
                        <Text style={[styles.label, { marginTop: 20, color: ADMIN_THEME.colors.primary }]}>Mother's Details</Text>
                        <View style={styles.row}>
                            <View style={styles.halfInput}>
                                <InputField label="First Name" placeholder="Mother Name" value={mother.first_name} onChangeText={(t: string) => setMother({ ...mother, first_name: t })} icon="person-outline" />
                            </View>
                            <View style={styles.halfInput}>
                                <InputField label="Last Name" placeholder="Surname" value={mother.last_name} onChangeText={(t: string) => setMother({ ...mother, last_name: t })} icon="person-outline" />
                            </View>
                        </View>
                        <InputField label="Phone" placeholder="Mobile Number" value={mother.phone} onChangeText={(t: string) => setMother({ ...mother, phone: t })} keyboardType="phone-pad" icon="call-outline" />
                        <InputField label="Occupation" placeholder="Designation" value={mother.occupation} onChangeText={(t: string) => setMother({ ...mother, occupation: t })} icon="briefcase-outline" />
                    ...
                    <Animated.View entering={FadeInDown.delay(300).duration(500)} style={styles.section}>
                        ...
                        <SelectField
                            ...
                            onSelect={(id: number) => setFormData({ ...formData, category_id: id })}
                            icon="list-outline"
                        />
                        ...
                        <SelectField
                            ...
                            options={RELIGIONS}
                            onSelect={(id: number) => setFormData({ ...formData, religion_id: id })}
                            icon="heart-outline"
                        />
                        ...
                        <SelectField
                            ...
                            onSelect={(id: number) => setFormData({ ...formData, blood_group_id: id })}
                            icon="water-outline"
                        />
                    </Animated.View>
