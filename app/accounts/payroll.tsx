...
export default function AccountsPayroll() {
    const {
        payrollData, loading, summary,
        selectedMonth, selectedYear, setSelectedMonth, setSelectedYear,
        fetchPayroll, markAsPaid
    } = usePayroll();
    ...
    const handleProcessPay = (item: PayrollEntry) => {
        Alert.alert(
            'Confirm Payment',
            `Mark ₹${item.net_salary.toLocaleString('en-IN')} as PAID for ${item.staff?.person?.first_name}?`,
            [
        ...
        <Animated.View entering={FadeInDown.delay(index * 50).duration(400)}>
            <TouchableOpacity style={styles.card} activeOpacity={0.9}>
                <View style={styles.headerRow}>
                    <Image
                        source={{ uri: item.staff?.person?.photo_url || 'https://cdn-icons-png.flaticon.com/512/3135/3135715.png' }}
                        style={styles.avatar}
                    />
                ...
                <View style={styles.footer}>
                    ...
                    {item.status === 'paid' ? (
                        <Text style={styles.dateText}>Paid on {item.payment_date}</Text>
                    ) : (
                        <TouchableOpacity
                            style={styles.payBtn}
                            onPress={() => handleProcessPay(item)}
                        >
        ...
        <View style={styles.container}>
            ...
            {loading ? (
                <View style={styles.centered}>
                    ...
                    <Text style={{ marginTop: 10, color: '#6B7280' }}>Generating payroll...</Text>
                </View>
            ) : (
                <FlatList
                    data={payrollData}
                    ...
                    ListEmptyComponent={
                        <View style={styles.centered}>
                            <Text style={{ color: '#9CA3AF' }}>No payroll records found.</Text>
                        </View>
                    }
                />
            )}
...
const styles = StyleSheet.create({
    ...
    // Summary
    summaryRow: { flexDirection: 'row', gap: 15, paddingHorizontal: 20, marginTop: 15 },
    summaryCard: { flex: 1, padding: 15, borderRadius: 12, alignItems: 'center' },
    summaryLabel: { fontSize: 12, color: '#6B7280', marginBottom: 5 },
    ...
    salaryBox: { backgroundColor: '#F3F4F6', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 },
    salary: { fontSize: 14, fontWeight: 'bold', color: '#1F2937' },
    ...
    payBtn: { backgroundColor: '#4F46E5', paddingVertical: 6, paddingHorizontal: 15, borderRadius: 8 },
