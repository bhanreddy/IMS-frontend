            ...
            <ScrollView contentContainerStyle={styles.scrollContent}>
                ...
                <View style={styles.summaryContainer}>
                    ...
                    <View style={styles.summaryItem}>
                        ...
                        <Text style={[styles.summaryValue, { color: '#10B981' }]}>₹{feeData?.summary.total_paid}</Text>
                    </View>
                    <View style={styles.summaryItem}>
                        <Text style={styles.summaryLabel}>Balance</Text>
                        <Text style={[styles.summaryValue, { color: '#EF4444' }]}>₹{feeData?.summary.balance}</Text>
                    </View>
                </View>
                        ...
                        <View key={fee.id} style={styles.feeCard}>
                            ...
                            <View style={styles.feeRow}>
                                ...
                                <View>
                                    ...
                                    <Text style={[styles.feeNum, { color: '#10B981' }]}>₹{fee.amount_paid}</Text>
                                </View>
                                {fee.discount > 0 && (
                                    <View>
                                        <Text style={styles.feeLabel}>Waiver</Text>
                                        <Text style={[styles.feeNum, { color: '#6366F1' }]}>-₹{fee.discount}</Text>
                                    </View>
                                )}
                                <View>
                                    <Text style={styles.feeLabel}>Due</Text>
                                    <Text style={[styles.feeNum, { color: '#EF4444' }]}>₹{fee.amount_due - fee.discount - fee.amount_paid}</Text>
                                </View>
                            </View>

                            <View style={styles.actionRow}>
                                <TouchableOpacity
                                    style={[styles.actionBtn, fee.status === 'paid' && styles.disabledBtn]}
                                    onPress={() => handlePayment(fee)}
                                    disabled={fee.status === 'paid'}
                                >
                                    <Ionicons name="cash-outline" size={18} color={fee.status === 'paid' ? '#9CA3AF' : '#10B981'} />
                                    <Text style={[styles.actionBtnText, { color: fee.status === 'paid' ? '#9CA3AF' : '#10B981' }]}>Collect</Text>
                                </TouchableOpacity>

                                <TouchableOpacity style={styles.actionBtn} onPress={() => handleAdjustment(fee)}>
                                    <Ionicons name="cut-outline" size={18} color="#6366F1" />
                                    <Text style={[styles.actionBtnText, { color: '#6366F1' }]}>Adjustment</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                    );
...
const styles = StyleSheet.create({
    ...
    studentCard: { backgroundColor: '#1F2937', padding: 20, borderRadius: 16, marginBottom: 20 },
    studentLabel: { color: '#9CA3AF', fontSize: 10, fontWeight: '800', marginBottom: 4 },
    studentName: { fontSize: 22, fontWeight: 'bold', color: '#fff' },
    studentId: { color: '#9CA3AF', fontSize: 12, marginTop: 4 },
    summaryContainer: {
        flexDirection: 'row',
        backgroundColor: '#fff',
        borderRadius: 16,
    ...
    summaryLabel: { fontSize: 11, color: '#6B7280', marginBottom: 5, fontWeight: '600' },
    summaryValue: { fontSize: 18, fontWeight: 'bold', color: '#111827' },
    sectionTitle: { fontSize: 14, fontWeight: '800', color: '#6B7280', marginBottom: 15, letterSpacing: 1 },
    feeCard: { backgroundColor: '#fff', borderRadius: 16, padding: 15, marginBottom: 15 },
    feeHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 15 },
    feeType: { fontSize: 16, fontWeight: 'bold', color: '#111827' },
    ...
    feeRow: { flexDirection: 'row', justifyContent: 'space-between', paddingBottom: 15, borderBottomWidth: 1, borderBottomColor: '#F3F4F6' },
    feeLabel: { fontSize: 10, color: '#6B7280', marginBottom: 3 },
    feeNum: { fontSize: 15, fontWeight: '700', color: '#1F2937' },
